...
class Bank:
    ...
    def deposit(self):
        for _ in range(100):
            amount = random.randint(50, 500)  # рандомное число для пополнения
            with self.lock:  # заблокируем доступ к балансу
                if self.balance < 500:
                    max_deposit = 500 - self.balance
                    actual_deposit = min(amount, max_deposit)
                    self.balance += actual_deposit
                    print(f"Пополнение: {actual_deposit}. Баланс: {self.balance}")

            time.sleep(0.001)  # задержку ставим
...
# Задание: Задача "Банковские операции":
#
